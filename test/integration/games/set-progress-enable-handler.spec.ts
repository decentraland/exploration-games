...
test('POST /games/progress/enable', ({ components }) => {
    let game
    let progress1
    ...
    beforeAll(async () => {
        ...
        await db.setProgressDisabled([progress1.id])
    })
    ...
    it('should return 200 when setting enable', async () => {
        ...
        const response = await makeRequest(localFetch, '/api/games/progress/enable', {
            method: 'POST',
    ...
    it('should return 400 when no auth', async () => {
        ...
        const response = await localFetch.fetch('/api/games/progress/enable', {
            method: 'POST',
            body: JSON.stringify(payload)
        })
