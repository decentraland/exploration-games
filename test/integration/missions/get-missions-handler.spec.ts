...
test('GET /api/missions', ({ components }) => {
  beforeAll(async () => {
    const { db } = components
    mission1 = await db.createMission('Mission Test1', VALID_CAMPAIGN_KEY, MissionType.MINI_GAMES, 'thumb_url')
    mission2 = await db.createMission('Mission Test2', VALID_CAMPAIGN_KEY, MissionType.MINI_GAMES, 'thumb_url')
    mission3 = await db.createMission('Mission Test3', VALID_CAMPAIGN_KEY, MissionType.MINI_GAMES, 'thumb_url')
    mission4 = await db.createMission('Mission Test4', VALID_CAMPAIGN_KEY, MissionType.MINI_GAMES, 'thumb_url')
