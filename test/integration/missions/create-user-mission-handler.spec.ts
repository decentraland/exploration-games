...
test('POST /api/missions/:id/start', ({ components }) => {
  ...
  beforeAll(async () => {
    ...
    mission = await db.createMission('Mission Test nueva', VALID_CAMPAIGN_KEY, MissionType.MINI_GAMES)
    mission2 = await db.createMission('Mission Test 2 nueva', VALID_CAMPAIGN_KEY, MissionType.MINI_GAMES)
