...
import { CustomDataKeys, IChallengeData, IScore, ValidCondition } from '../../types'

export function isScoreMetCondition(
  scoreData: IScore,
    ...
    switch (key) {
      ...
      case 'time': {
        ...
        if (key in scoreData && value !== undefined && value !== null && typeof value === 'number') {
          ...
          logger.warn(`Can't find key: ${key} in score.`)
        }
        break
      }
      case 'data': {
        if (key in scoreData && scoreData[key] !== undefined && scoreData[key] !== null) {
          ...
          if (customKey && scoreData.data && customKey in scoreData.data && value && value[customKey]) {
            ...
            logger.warn(`Can't find key: ${customKey} in score custom data.`)
          }
        } else {
          logger.warn(`Can't find score custom data.`)
        }
        break
      }
      default:
        logger.warn(`No score type key match: ${key}`)
        break
    }
